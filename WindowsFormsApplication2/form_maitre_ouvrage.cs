...
{
    public partial class form_maitre_ouvrage : Form
    {
        public form_maitre_ouvrage()
        {
            InitializeComponent();
        }
        Sqlcon c = new Sqlcon();
        public void edit_post()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand(" update Maitre set Intitulé ='" + txt_intitile.Text + "' , Adresse ='" + txt_adress.Text + "' , Telepohne ='" + txt_tel.Text + "' , Fix ='" + txt_fix.Text + "' , Email ='" + txt_email.Text + "' , website ='" + txt_siteweb.Text + "' where Idmaitre = '" + id + "'", c.cnx);
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
            catch { }
        ...
        public void add_arch()
        {
            try
            {
                SqlCommand cmd = new SqlCommand(" insert into Maitre values ('" + txt_intitile.Text + "','" + txt_adress.Text + "','" + txt_tel.Text + "','" + txt_fix.Text + "','" + txt_email.Text + "','" + txt_siteweb.Text + "')");
                cmd.Connection = c.cnx;
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
        ...
        public void remove_post()
        {
            ...
            {
                int id = (int)dataGridView1.Rows[i].Cells[0].Value;
                SqlCommand cmd = new SqlCommand("delete from Maitre where Idmaitre = '" + id + "'", c.cnx);
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
            catch { }
