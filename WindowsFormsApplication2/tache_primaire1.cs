...
{
    public partial class tache_primaire1 : Form
    {
        public tache_primaire1()
        {
            InitializeComponent();
        }
        Sqlcon c = new Sqlcon();
        ...
        {
            bool b = false;
            if (textBox2.Text == "" && comboBox1.SelectedValue == null) b = true;
            return b;
        }
        ...
        public void refresh_dgv()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("select * from tache_pre");
        ...
        public void add_cat()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("insert into tache_pre values ('"+comboBox1.SelectedValue+"','" + textBox2.Text + "')");
                cmd.Connection = c.cnx;
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
        ...
        public void fill_tach_g()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("select * from Tache_glob ", c.cnx);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                comboBox1.DataSource = dt;
                comboBox1.DisplayMember = "intitule";
                comboBox1.ValueMember = "idtachglob";
            }
            catch
            { }
        }
        ...
        public void remove_cat()
        {
            ...
            SqlCommand cmd = new SqlCommand("delete from tache_pre where id = '" + id + "'", c.cnx);
            cmd.ExecuteNonQuery();
            refresh_dgv();
            }
            catch { }
        ...
        public void edit_cat()
        {
            ...
            {
                int id = (int)dataGridView1.Rows[i].Cells[0].Value;
                SqlCommand cmd = new SqlCommand("update tache_pre set desc_ = '" + textBox2.Text + "' , id_tachglob='"+comboBox1.SelectedValue+"' where id =  '" + id + "'", c.cnx);
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
            catch { }
