...
{
    public partial class frm_tache_glob : Form
    {
        public frm_tache_glob()
        {
            InitializeComponent();
        }
        Sqlcon c = new Sqlcon();
        ...
        public void refresh_dgv()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("select * from Tach_glob");
        ...
        public void add_cat()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("insert into Tach_glob values ('" + textBox2.Text + "')");
                cmd.Connection = c.cnx;
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
        ...
        public void remove_cat()
        {
            ...
            SqlCommand cmd = new SqlCommand("delete from Tach_glob where idtachglob = '" + id + "'", c.cnx);
            cmd.ExecuteNonQuery();
            refresh_dgv();
            // }catch { }
        ...
        public void edit_cat()
        {
            ...
            {
                int id = (int)dataGridView1.Rows[i].Cells[0].Value;
                SqlCommand cmd = new SqlCommand("update Tach_glob set intitule = '" + textBox2.Text + "' where idtachglob =  '" + id + "'", c.cnx);
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
            catch { }
