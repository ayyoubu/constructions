...
{
    public partial class form_unite : Form
    {
        public form_unite()
        {
            InitializeComponent();
        ...
        public void refresh_dgv()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("select * from Unite");
        ...
        public void add_cat()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("insert into Unite values ('" + textBox2.Text + "')");
                cmd.Connection = c.cnx;
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
        ...
        public void remove_cat()
        {
            ...
            SqlCommand cmd = new SqlCommand("delete from Unite where idunite = '" + id + "'", c.cnx);
            cmd.ExecuteNonQuery();
            refresh_dgv();
            // }catch { }
        ...
        public void edit_cat()
        {
            ...
            {
                int id = (int)dataGridView1.Rows[i].Cells[0].Value;
                SqlCommand cmd = new SqlCommand("update Unite set intitule = '" + textBox2.Text + "' where idunite =  '" + id + "'", c.cnx);
                cmd.ExecuteNonQuery();
                refresh_dgv();
            }
            catch { }
